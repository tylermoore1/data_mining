...
# new_data = data.groupby(['match_id','team_id']).agg({'player_kills' : 'sum',
...
party_size = data.groupby(['match_id','team_id'])[['party_size']].mean()
...
result = pd.concat([game_size, party_size, player_dist_ride, player_dist_walk, player_kills, team_placement], axis=1)
